...
public interface OrderRepository extends JpaRepository<Order, String> {
    ...
    @Query(value = "select COUNT(*) from `order` o where exists (select count(1) from order_item oi left join product p on oi.product_id = p.id where oi.order_id=o.id and p.mch_id=?1) AND o.state='A'", nativeQuery = true)
    Integer unreadCount(String mchId);
}
